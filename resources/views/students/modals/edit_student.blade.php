...
<div class="modal-body">
  	<form action="{{ url('student/'.$id) }}" method="POST" id="edit_teacher_form_id">
  		@csrf
      ...
      <div class="form-group" style="margin-top: 1%;">
        ...
        @isset($courses)
            @foreach ($courses as $course)
                <option value="{{$course->id}}" <?php if($course->id == $course_id) echo "selected";?>> {{ $course->class_name }} </option>
            @endforeach  
          @endisset 
          </select>
      </div>
