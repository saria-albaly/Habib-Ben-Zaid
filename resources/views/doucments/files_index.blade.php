      ...
      <div class="box-body">
        <table id="example1" class="table table-bordered table-striped" style=" width: 100%; direction:rtl;text-align: right">
          ...
          <tr>
            ...
            <th style="direction:rtl;text-align: right">اسم الملف</th>
            ...
            <th style="direction:rtl;text-align: right">تاريخ آخر تعديل</th>
              ...
              @if($privCons->count() > 0)
                <tr>
                  ...
                  <td>
                    ...
                    @endif
                    <!-- @if($document->doc_type_code == 'image')
                    <a type="button" class="btn btn-primary btn-flat" title="عرض الصورة" href="{{url('document/'.$document->id.'/paraghraphs')}}"><i class="fa fa-eye"></i></a>
                    @endif -->
                    @if($privCons->filter(function($r){return $r->action_name == 'edit_request';})->count() > 0)
                      @if( $document->view_edit_request_btn )
                        <button type="button" class="btn btn-success btn-flat" title="طلب تعديل " onclick="createEditRequest({{$document->id}})"><i class="fa fa-ticket"></i></button>
                      ...
                      @if($document->view_edit_request_btn_pending)
                        <form action="{{url('DocumentEditRequest')}}/1?document_id={{$document->id}}&ref=files" method="POST" style="display: contents;">
                          {{ method_field('DELETE') }}
                          @csrf
                    ...
                    @if($privCons->filter(function($r){return $r->action_name == 'download_pdf';})->count() > 0)    
                      <a type="button" class="btn btn-info btn-flat" title="تحميل الملف"  href="{{url('document/download/'.$document->id)}}"><i class="fa fa-download"></i></a>
                    @endif 
                    <!-- <a type="button" class="btn btn-defualt btn-flat" title="طباعة الوثيقة " href="{{url('test/'.$document->id)}}"><i class="fa fa-print"></i></a> -->
                    @if(isset($grant['115']) && $grant['115'] || ($privCons->filter(function($r){return $r->action_name == 'delete' && $r->array_section_ids==null;})->count() > 0))
...
<script type="text/javascript">
  function view_create_doc_modal() {
    ...
    xhttp.open("GET", "{{url('document/create?type=file')}}" , true);
    xhttp.send(); 
  }
...
<script type="text/javascript">
    function createEditRequest(documentID){
      // body...
      var xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function(id) {
        if (this.readyState == 4 && this.status == 200) {
      ...
      xhttp.open("GET", "{{url('DocumentEditRequest/create?documentID')}}="+documentID , true);
      xhttp.send(); 
    }
